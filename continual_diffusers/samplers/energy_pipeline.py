...
class Energy_Text_DDPMPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            model_output,image = (
                model_output.detach() ,image.detach()
            )  # detach to avoid backprop through scheduler
            ...
            if self.mcmc_sampler is not None:
                ...
                if t >= mcmc_sampler_start_timestep:
                    ...
                    kwargs = {"scalar": scalar, "class_labels": prompt_embeds}
...
class Energy_DDPMPipeline(DiffusionPipeline):
    r"""
    Pipeline for image generation.
    ...
    def __call__(
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            if classifier_free_guidance:
                ...
                final_labels = torch.cat([class_labels, null_labels], dim=-1)
                ...
                image = torch.cat(
                    [image] * 2
                )  # Duplicate the image for the two class labels
                model_output = self.unet(
