...
class Energy_StableDiffusionPipeline(
    DiffusionPipeline,
    StableDiffusionMixin,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred, energy_norm = self.unet(
                    latent_model_input,
                    t,
                    ...
                    return_dict=False,
                    eval_mode=True,  # For EBM inference
                )
...
class Compose_MCMC_StableDiffusionPipeline(
    DiffusionPipeline,
    StableDiffusionMixin,
    TextualInversionLoaderMixin,
    LoraLoaderMixin,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # For compositions we expect multiple prompts separated by '|'
        if "|" in prompt:
        ...
        if isinstance(prompt, list) and batch_size == 1:
            # remove extra unconditional embedding (only retain one unconditional embedding)
        ...
        if mcmc_sampler is not None:
            ...
            def gradient_fn(x, t, model_unet, model_args):
                ...
                x = torch.cat([x] * 2) if self.do_classifier_free_guidance else x
                x = self.scheduler.scale_model_input(x, t)

                noise_pred = []
                for j in range(text_embed.shape[0]):
                    noise_pred.append(
                        model_unet(
                            x[:1],
                            t,
                            encoder_hidden_states=text_embed[j : j + 1],
                            timestep_cond=timestep_cond,
                            cross_attention_kwargs=self.cross_attention_kwargs,
                            added_cond_kwargs=added_cond_kwargs,
                            return_dict=False,
            ...
            mcmc_sampler._gradient_function = gradient_fn
            ...
            for i, t in enumerate(timesteps):
                ...
                if mcmc_sampler is not None:
                    if t > mcmc_sampler_start_timestep:
                        ...
                        latents = mcmc_sampler.step(latents, t, self.unet, model_args)
...
class Custom_StableDiffusionPipeline(
    DiffusionPipeline,
    StableDiffusionMixin,
    TextualInversionLoaderMixin,
    LoraLoaderMixin,
